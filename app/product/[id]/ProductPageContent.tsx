...
import { dietaryAddons, cakeFillings, type Product } from '@/lib/products';
...
export default function ProductPageContent({ product, relatedProducts }: Props) {
  ...
  const storageKey = `product-selections-${product.id}`;
  ...
  useEffect(() => {
    ...
    setInitialized(true);
  }, [editingItem, initialized, storageKey, product.category, product.minOrder]);
